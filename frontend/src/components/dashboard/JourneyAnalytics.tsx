...
import { fetchJourneyData } from '@/lib/dashboard-utils';
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        const journeyData = await fetchJourneyData(supabase, period);
        setData(journeyData);
      } catch (error) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold text-foreground-primary">Customer Journeys</h2>
          <p className="text-foreground-tertiary mt-1">Track the path from freebie to purchase</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KPICard
          ...
          subtitle="Of freebie signups who purchase"
          loading={loading}
        />
        <KPICard
