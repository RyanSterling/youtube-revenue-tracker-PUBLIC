...
import { fetchFreebieData } from '@/lib/dashboard-utils';
...
export function FreebieAnalytics() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        const freebieData = await fetchFreebieData(supabase, period);
        setData(freebieData);
      } catch (error) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold text-foreground-primary">Freebie Analytics</h2>
          <p className="text-foreground-tertiary mt-1">Track lead magnet and quiz performance</p>
        </div>
